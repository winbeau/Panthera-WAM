...
- **零修改\* 的边界**：官方 SDK 源码零修改，但 armd **不能**直接调用 SDK 的阻塞式方法（`iswait=True` 等待、`moveL()`、`Recorder.play()`）——这些方法会把唯一的硬件线程钉死数秒到整条轨迹，破坏可抢占安全层。armd 改为**用 SDK 的公开规划/控制原语（`compute_cartesian_path` / `septic_interpolation` / `pos_vel_tqe_kp_kd` / `check_position_reached`）在自己的控制循环里逐周期步进**。这一点是本计划相较初稿最重要的修订（详见 Part 1 §1）。
...
HardwareLoop 是唯一持有 `Panthera` 对象、独占 7×USB 的线程，以固定周期运行（目标 200–500 Hz，实际上限由 SDK 电机通信速率决定，M0 实测锁定）。**每个周期严格按序**：
...
| `cartesian movel`（Mode B） | 直接调 `moveL()` 单体阻塞 | armd 自建执行循环：`compute_cartesian_path → compute_time_parameterization → smooth_trajectory_spline` 得到逐点轨迹，HardwareLoop 每周期发一个点（`pos_vel_tqe_kp_kd`），每点发布 `fraction`、每点查 cancel/estop。**不调 `moveL()`** |
...
> **零修改边界更新**：`_execute_trajectory`（moveL 内部执行循环）、`_prepare_playback_frames` / `_moving_average`（play 内部预处理）是 SDK 私有方法。armd 自建执行循环需**复现其逻辑**（不 import 私有名，而是用公开原语等价重写），并在实现前对拍验证与 SDK 单体调用行为一致。这类「靠公开原语拼装、需自建执行循环」的能力单列于 §4b，与 `GetGravity` 这种一对一封装区分。

### 1.3 EStop 抢占的物理可行性（回填 G2）

EStop 之所以能「≈100ms 生效」，正是因为 §1.1 保证了 HardwareLoop 永不长阻塞：asyncio 层收到 `EStop` 只置一个 estop 标志，HardwareLoop 在**下一个控制周期开头**（§1.1 步骤 1）就读到并执行 `set_stop()`/保位。最坏延迟 = 一个控制周期 + 一次电机写 ≈ 数毫秒到十几毫秒，远小于 100ms。**不采用**从 asyncio 线程直接触碰 USB 的旁路写通道（那会破坏单线程独占、与循环 I/O 打架）。这是「方案 (a)：保证循环永不长阻塞」的落地。

### 1.4 纯计算分流与 IK 让路（回填 G8）

- **电机 I/O 类调用**（`get_current_*`、`check_position_reached`、所有运动下发）：一律 marshal 到 HardwareLoop 队列串行执行，无并发。
- **纯计算类调用**（FK/IK/Jacobian/可操作度/动力学/`compute_cartesian_path` 预览）：只依赖 pinocchio 模型、不碰电机，放在独立计算 worker，用**专用的第二个 pinocchio 模型实例**（启动时构造/克隆），与 HardwareLoop 使用的模型物理隔离，从而不与电机 I/O 竞争、也不阻塞循环。
- **IK 让路**：`inverse_kinematics(multi_init=True)` 秒级 CPU 密集，计算 worker 给它**墙钟超时预算**（默认 5s，可配），超时返回 `found=false, timeout=true`，绝不占死资源。
- **待核实（§8 开放项）**：pinocchio 模型能否安全地独立双实例化。若 SDK 内部模型不可克隆/共享不安全，则退化方案为「计算也 marshal 回 HardwareLoop，但每调用带超时预算」，代价是 IK 期间控制周期抖动——M0 阶段实测二选一。
...
|---|---|---|---|
| **EStop** | `set_stop()`（或立刻改发「当前位保位」目标） | `set_stop()` + 前馈/速度归零 | `set_stop()` + 标记 execution=CANCELLED |
...
> 「安全收尾/减速停止」的具体算法（按剩余轨迹限加加速度减速 vs 回最近安全点）在 M4/M7 实现前单独定案（§8 开放项）。
...
| `trajectory run-waypoints`（多点轨迹执行） | 示例 `3_interpolation_control`：septic 插值 + 逐点下发 | `septic_interpolation[_with_velocity]` 生成轨迹，HardwareLoop 逐点 `pos_vel_tqe_kp_kd`，每点发 `fraction`/查取消 |
...
| 28 | `moveL` | v1 | `cartesian movel` | `MoveL` | B | **armd 自建执行循环**（§1.2），不调 `moveL()`；`StreamExecution` 观察 `fraction`，`CancelExecution` 减速收尾 |
...
> 现状：已产出 3 份可交互 HTML 视觉稿（`mockups/mockup-A-fluent-console.html`、`docs/mockups/mockup-B-fluent-cards.html`、`mockups/mockup-C-fluent-cockpit.html`），供选型。三者共享同一套 Fluent 设计语言与数据契约，差异在信息密度与呈现隐喻。
...
### 稿 C — Fluent 驾驶舱（`mockup-C-fluent-cockpit.html`）
...
- **M0-1 非阻塞循环 + 可抢占 EStop**：HardwareLoop 逐周期步进一个 `joint move`，运动中途置 estop 标志，实测下一周期 `set_stop` 生效、总延迟 < 100ms。
- **M0-2 自建 moveL 执行循环**：用 `compute_cartesian_path`+逐点 `pos_vel_tqe_kp_kd` 复现一条直线，产出单调 `fraction`、中途可取消，与 SDK `moveL()` 末端误差对拍一致。
...
- **M-W1 只读监控**：三稿之一落地关节监控 + `state watch` 实时刷新（无需 lease）。
- **M-W2 控制闭环**：获取控制权 + jog + `joint move/movej` + 夹爪 + 常驻 EStop 条 + moveL 进度/取消。
- **M-W3 主题与选型**：三态主题切换；基于 A/B/C 试用结论定稿一款（建议：交付用 B 卡片式，调试期保留 A）。
...
1. **pinocchio 模型可否安全双实例化**（§1.4）：决定「计算 worker 独立模型」还是「marshal+超时预算」退化方案。M0-3 实测。
2. **`set_reset_zero()` 是否产生运动**（§5.11 S2）：决定 `SetZero` 用模式 A 还是 B、confirm 口径。核对 `.pyi`/示例。
3. **继承层签名**（`set_stop`/`set_reset_zero`）：`EStop`/`SetZero` 的 request/response 现为最小占位，需核对 `htr.Robot` 桩文件/`bindings.cpp`。
4. **`MotorState` 字段**：`get_current_state` 真实电机对象字段（温度/电压/使能位？）未知，proto 为保守占位，实现阶段补全或裁剪。
5. **私有方法逻辑复现**（§1.2）：`_execute_trajectory` / `_prepare_playback_frames` / `_moving_average` 的等价重写需与 SDK 单体调用对拍验证。

## 设计待定案

6. **安全收尾算法**（§4）：`CancelExecution`/watchdog 触发时「按剩余轨迹限加加速度减速」vs「回最近安全点」，M4/M7 前定案。
7. **控制周期频率**（§1.1）：目标 200–500 Hz，受 SDK 电机通信上限约束，M0 实测锁定。
