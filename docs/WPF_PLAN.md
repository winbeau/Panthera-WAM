...
> `armd:50051`、D405 `camerad:50052` 与 C920e `CameraService:50053`；此模式不启动
> WSL TCP bridge，也不执行 usbipd/WSL
> 环境引导。另增 `SshRemote` 模式：用户显式填写 OpenSSH 基础信息后，WPF 自动识别
> Pi/WSL 架构、已部署仓库和启动入口，并通过 localhost SSH 隧道连接三个 gRPC 服务。
...
> 六轴 Panthera-HT、D405 与 C920e 由同一 Raspberry Pi 5 后端控制，`armd:50051`、
> D405 `camerad:50052` 与 C920e `camerad:50053` 分进程独占硬件，WPF 分别连接三个
> gRPC 端点。
...
│ Windows 宿主                 │  Pi IP:50051 / :50052 / :50053     │ Raspberry Pi 5                │
│ ┌─────────────────────┐     │                                   │ ┌──────────────────────────┐ │
│ │ WPF 可视化终端       │◄────┼───────────────────────────────────┼─┤ armd → Panthera-HT       │ │
│ │ panthera-cli 也可并存 │     │                                   │ ├──────────────────────────┤ │
│ └─────────────────────┘     │                                   │ │ camerad → RealSense D405 │ │
└─────────────────────────────┘                                   │ ├──────────────────────────┤ │
                                                                    │ │ camerad → Logitech C920e │ │
                                                                    │ └──────────────────────────┘ │
                                                                    └──────────────────────────────┘
```

WPF 终端与 `panthera-cli` 都分别连接机械臂和两项相机服务；只有 `armd:50051` 的
机械臂控制 RPC 使用 `AcquireControl` 互斥语义，`50052/50053` 均为只读采集端点。
...
- 探测和启动成功后，配置保存为 `BackendMode=SshRemote`。WPF 自动重启，保持
  `ssh -N` 三端口转发：Windows `127.0.0.1:50050 → 远端 127.0.0.1:50051`、
  `127.0.0.1:50049 → 远端 127.0.0.1:50052`、
  `127.0.0.1:50048 → 远端 127.0.0.1:50053`。因此无需猜测远端网卡地址，也不会要求
  armd/两项相机服务暴露到公网接口。若远端是只部署旧双服务的 WSL，向导应明确标记
  “俯视相机不可用”，而不是把整条机械臂/D405 链路判为失败。
- 整个流程只建立连接和启动服务，绝不获取控制权、使能或发送运动指令；成功后的应用
  重启先走正常关闭流程，释放现有 lease 和点动状态，再启动新实例。

### 3.9 C920e 俯视相机与三端点客户端

**端点与兼容性**

- `TerminalSettings.CameraEndpoint` 继续表示 D405 `50052`，避免破坏已安装版本的 JSON；
  新增 `OverheadCameraEndpoint`，Remote 默认 `http://<Pi IP>:50053`，`SshRemote` 本地默认
  `http://127.0.0.1:50048`。旧 WSL 回退模式不自动假定 C920e 位于本机 WSL。
- SSH 隧道固定映射：`50050→50051`、`50049→50052`、`50048→50053`。探测向导只有在
  三端口均监听后才报告“完整后端已启动”；若仅相机失败，步骤必须指出具体端口和设备。
- 旧设置文件缺少 `OverheadCameraEndpoint` 时使用模式相关默认值，不改写或误用现有
  `CameraEndpoint`。

**客户端模型与后台管线**

- 增加 `CameraSourceKind.Wrist/Overhead`；`CameraFrameSnapshot` 和 `CameraSnapshot` 带来源，
  `LatestCameraFrames` 以 `(source, stream)` 为键，避免两台相机的 color 序号相互覆盖。
- D405 后台泵保持 color+depth，C920e 后台泵只订阅 color/JPEG。三条流分别独立重连和
  限流；C920e 默认预览 10–15fps，D405 延续当前 15fps，UI 仍统一按 30fps latest-wins
  渲染。
- `camera.proto` 的角色用于启动校验：D405 端点不是 `WRIST` 或 C920e 端点不是
  `OVERHEAD` 时，UI 显示“端点角色配置错误”并停止该流，不能将画面静默放错位置。
- C920e JPEG 用 `BitmapImage`/缓存流在后台解码后冻结，再交给 UI；D405 RGB8/Z16
  保持现有 `WriteableBitmap` 路径。JPEG 已压缩，不再叠加 gRPC gzip。
...
- 右列三格均保持 1:1；D405 与运动控制处于同一行。
- 使用 WPF 原生 `Grid + GridSplitter`。用户只调整右列宽度，CAD 两个大视图高度、
  C920e/D405 方格和运动控制高度随同一列宽联动，避免产生互不对齐的自由尺寸。
- 俯视预览显示 FPS、帧龄、分辨率与状态；断流时保留最后一帧但叠加“陈旧/断开”标识，
  不把旧画面伪装成实时画面。

**LingBot-VA 边界**

- WPF 只预览并触发录制，不将 Windows 收到帧的时间作为数据时间戳，也不直接写训练集。
- Pi 本地采集器使用 `captured_monotonic_ns` 对齐机械臂状态、C920e 和 D405；WPF 仅展示
  同步偏差、丢帧率和录制状态。
...
2. **远程链路边界**：主路径依赖 Windows 到 Pi IP 的稳定连通；当前 gRPC 为明文 HTTP/2，
   应使用 Tailscale 或受信 LAN，并限制 50051/50052/50053 的来源。WSL localhost bridge 仅是兼容回退。
...
8. **后端启动方式**：Pi 主路径优先使用 systemd user service 管理 `armd:50051`、
   `camerad:50052` 与 `overhead-camera:50053`；WSL 回退保留已有双服务能力。
   `Remote` 模式仍只负责直连探活；用户显式选择
