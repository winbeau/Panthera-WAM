...
> **无控制权空闲策略**：armd 仍以 200Hz 发送完整 MIT 帧满足共享 CAN 与固件看门狗契约。六轴使用 `kp=0 / kd=0`，先对离散速度反馈做 40ms 一阶低通，再以限幅力矩 `τ=-D·v_filtered` 实现软件柔顺阻尼；静止时力矩自然收敛到零。夹爪保持零力矩。这与急停恢复阻尼、夹爪动作期间的固件阻尼、运动末点保位严格分离，避免固件 `KD` 直接放大速度量化台阶造成徒手拖动顿挫；显式零阻尼被动模式仍作为内部安全回退保留。
