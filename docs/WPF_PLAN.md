...
> 范围声明：本文档只覆盖 **客户端**（Windows WPF 可视化终端）一侧的设计，不涉及 armd / gRPC 服务端实现代码，也不写任何 C#/XAML 代码，仅做架构与页面/交互的详细规划。前置架构决策（六轴 Panthera-HT 与 D405 均由同一 WSL2 后端控制，`armd` 与 `camerad` 分进程独占硬件，WPF 只连接 `armd` 单一公开 gRPC 端点，HardwareLoop 单线程，v1=关节监控+jog+moveJ/moveL）视为既定事实，不重复论证。
...
│  Windows 宿主                │  HTTP/2, localhost:50051 (WSL2      │  WSL2                         │
│  ┌─────────────────────┐    │  自动端口转发, 无需手动配置 NAT)     │  ┌─────────────────────────┐  │
│  │ WPF 控制终端 (本计划) │◄───┼──────────────────────────────────┼─▶│ armd (Python 守护服务)   │  │
│  │ panthera-cli 也可并存 │    │                                    │  │ HardwareLoop 独占 Panthera│  │
│  └─────────────────────┘    │                                    │  │ 对象, usbipd 挂载 7×USB   │  │
└─────────────────────────────┘                                    │  └─────────────────────────┘  │
                                                                     └──────────────────────────────┘
```

WPF 终端与 `panthera-cli` 是 armd 的两个平行 gRPC 客户端，共享同一套 `AcquireControl` 互斥语义 —— 这是错误处理章节（第 5 节）里"控制权被抢占"场景的直接来源。
...
| 6 | 启动 WSL 后端并联通 | gRPC `daemon status` / camera status 探活 | `wsl -d <distro> -u <user> -- systemctl --user start camerad armd`，随后通过 `armd` 公开端点轮询直到机械臂与相机均健康 | 否 |
...
2. **WSL2 localhost 转发假设**：WPF 终端默认假设 Windows 侧访问 `localhost:50051` 能直达 WSL2 里的 armd（WSL2 的自动 localhost 转发机制）；若用户网络策略/WSL 版本导致转发失效，设置里的"手动端点"是唯一兜底，需要在 M0 阶段实际验证一次，而不是纸面假设。
...
8. **WSL 后端启动方式已定案**：3.7 节步骤 6 使用 systemd 用户服务同时启动 `camerad` 与 `armd`；`armd` 对外提供唯一端点并代理内部 CameraService。开发/恢复场景也可调用仓库提供的 `panthera-up` 完成同样的检查与启动流程。
