...
def main() -> int:
    ...
    print(
        f"trajectory：{trajectory_output.name}（7 轴 measured state/torque + timestamp/sequence）", flush=True
    )
    ...
    if bounds is not None:
        ...
        actual_duration_s = max(
            0.0,
            (int(last["sampled_monotonic_ns"]) - int(first["sampled_monotonic_ns"])) / 1_000_000_000.0,
        )
