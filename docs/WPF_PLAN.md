...
> **v2.1 紧凑 Fluent 重构（2026-07-19）**：控制页调整为“宽 CAD 三视图 + 296px 紧凑控制轨”。删除左侧重复的 6 轴状态卡和 TCP 卡，六轴、夹爪、TCP 与模型状态统一由三视图内嵌实时状态区呈现；右侧保留夹爪、6 轴 hold-to-run Jog、速度与控制权全部功能。CAD 宽度 ≥980px 时采用“侧视/主视大列 + 俯视/状态窄列”，低于 980px 自动切换 2×2 布局，确保 1240×800 下侧视与主视仍有足够宽度。Jog 改为六条横向控制行并内嵌软限位进度，最小 UI 缩放锁定 90%，避免旧布局遗留的 75%–85% 小字状态。浅色 100%、深色 90%、1600×960 与 1240×800 均已通过实际 WPF 窗口截图验收，功能与键盘可达性不减少。
...
> 范围声明：本文档只覆盖 **客户端**（Windows WPF 可视化终端）一侧的设计，不涉及 armd / gRPC 服务端实现代码，也不写任何 C#/XAML 代码，仅做架构与页面/交互的详细规划。前置架构决策（六轴 Panthera-HT 与 D405 均由同一 WSL2 后端控制，`armd:50051` 与 `camerad:50052` 分进程独占硬件，WPF 分别连接两个 gRPC 端点，HardwareLoop 单线程，v1=关节监控+jog+moveJ/moveL）视为既定事实，不重复论证。
...
│ Windows 宿主                 │  localhost:50051 / :50052         │ WSL2                          │
│ ┌─────────────────────┐     │                                   │ ┌──────────────────────────┐ │
...
8. **WSL 后端启动方式已定案**：3.7 节步骤 6 使用 systemd 用户服务同时启动 `camerad:50052` 与 `armd:50051`。开发/恢复场景也可调用仓库提供的 `panthera-up` 完成同样的检查与启动流程。
