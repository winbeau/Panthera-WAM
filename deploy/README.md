...
机械臂、D405 与 C920e 直接插在 Pi 5 上。Python 依赖全部由 `uv` 管理；机械臂 SDK 与
...
sudo apt-get update
sudo apt-get install -y build-essential cmake libssl-dev libusb-1.0-0-dev pkg-config v4l-utils
...
$env:PANTHERA_OVERHEAD_CAMERA_ENDPOINT = "http://<PI_IP>:50053"
Panthera.Terminal.App.exe
```

也可编辑 `%LOCALAPPDATA%\Panthera\terminal-settings.json`，设置 `BackendMode`、
`Endpoint`、`CameraEndpoint` 和 `OverheadCameraEndpoint`。`Remote` 模式不会启动 WSL
bridge，也不会调用 usbipd；环境引导探测远程 armd、D405 与 C920e 三个端点。

C920e 由独立的 `overhead-camera.service` 提供 `:50053`，只打开稳定别名
`/home/winbeau/camera-devices/c920e`，保持设备原生 `1920×1080 MJPEG@30`，不会固定
`/dev/videoN`。只读检查：

```bash
systemctl --user start overhead-camera
PANTHERA_OVERHEAD_CAMERA_ENDPOINT=127.0.0.1:50053 \
  uv run panthera camera status --source overhead --json
PANTHERA_OVERHEAD_CAMERA_ENDPOINT=127.0.0.1:50053 \
