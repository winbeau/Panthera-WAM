...
| **Watchdog 超时 / lease 释放** | 活动运动先执行安全取消；结束后切入零力矩、零刚度、零阻尼的被动空闲帧 | 活动运动先归零/取消；结束后切入同一被动空闲帧 | 触发 `CancelExecution` 的安全减速收尾，完成后进入被动空闲 |
...
> **无控制权空闲策略**：armd 仍以 200Hz 发送完整 MIT 帧满足共享 CAN 与固件看门狗契约，但六轴和夹爪均使用 `torque=0 / kp=0 / kd=0`。这与急停恢复阻尼、夹爪动作期间的关节阻尼、运动末点保位严格分离，避免操作员未持有 lease 时徒手拖动仍被周期性阻尼抵抗。
...
| `estop trigger` / `calibrate zero` | 继承层 `set_stop()` / `set_reset_zero()`（签名已由 `bindings.cpp` 核实） | 直通/单次调用 |
...
`set_stop()` / `set_reset_zero()` 来自 `htr.Robot` 父类，不在 42 方法清单内；签名已由 `panthera_python/src/bindings.cpp` 与 C++ 声明核实：

| # | 方法 | 阶段 | CLI | rpc | 备注 |
|---|---|---|---|---|---|
| S1 | `set_stop()` | v1 | `estop trigger` | `EStop` | 最高优先级，经 estop 标志在下一控制周期抢占（§1.3）；不需持锁（§3.3） |
| S2 | `set_reset_zero()` / `set_reset_zero_motors(ids)` | v1 | `calibrate zero` | `SetZero` | 返回 `void`；重定义当前位置为零点，不产生目标运动。指定电机接口接收零基索引，RPC 的 1..7 在 RealBackend 内减一；仍需 confirm |
...
panthera control heartbeat [--interval FLOAT=0.5]          # 显式前台维持 lease，Ctrl+C 停止
...
panthera execution watch EXECUTION_ID                    # 只读观察长动作
panthera execution cancel EXECUTION_ID                   # 需 lease，触发安全减速收尾
...
10. **D405 视频流 / LeRobot 导出**：非 `Panthera`/`Recorder` 方法，不计入 42 项清单；正式契约已独立为 `camera.proto`/`dataset.proto`，`arm.proto` 占位已删除。
...
- SDK 源码使用 public fork `winbeau/Panthera-HT_SDK`，以 git submodule 固定在 `vendor/Panthera-HT_SDK`；上游保持 `HighTorque-Robotics/Panthera-HT_SDK`，主仓库不直接修改 SDK 源码。wsl-host 可继续保留 `~/Panthera-HT_SDK` 作为旧运行副本，但默认优先使用 vendor。
