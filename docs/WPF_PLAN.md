...
> 环境引导。本文后续 WSL 引导内容保留为 `WslBridge` 兼容模式的历史设计。
...
8. **后端启动方式**：Pi 主路径与 WSL 回退都使用 systemd user service 管理
   `camerad:50052` 与 `armd:50051`；WPF 的 `Remote` 模式只负责探活，不通过 SSH 远程启停服务。
