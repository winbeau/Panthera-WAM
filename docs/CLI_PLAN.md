...
- **M8 相机流与 LeRobot 导出**：`camerad:50052` 采集与 `camera status/snapshot/stream` 已落地；`armd:50051` 独立提供机械臂服务，`dataset export-lerobot` 待实现。
  验收：D405 与机械臂同时 attach 到同一 Linux，状态、快照与持续帧流通过独立 CameraService 端点访问；LeRobot 字段映射与导出标准留给 `dataset.proto` 计划。
