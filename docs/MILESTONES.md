...
- [ ] **M8 相机流 + LeRobot 导出（进行中）**：`camera stream`、`dataset export-lerobot`
  ...
  - [x] 独立 `camera.proto`、armd 专用 CameraWorker、gRPC 状态/快照/帧流，以及 `camera status/snapshot/stream` CLI 已接入；CI 使用仿真相机
  - [x] WPF 环境引导按 `VID_8086&PID_0B5B` 自动发现并 attach D405；部署加入 librealsense udev 规则与工作流文档
