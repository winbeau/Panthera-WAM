...
1. **pinocchio 模型可否安全双实例化**（§1.4）：**静态已结案**——`_load_urdf_model` 用 `pin.buildModelFromUrdf` 逐实例构造、无全局/共享态，双实例化安全；且 `get_Gravity` 会临时改写 `model.gravity` 再还原（共享模型下即数据竞争），**反向证明必须用独立第二实例**。运行时抖动仍由 M0-3 实测。
...
5. ~~**私有方法逻辑复现**~~ → **算法已全部提取**，见核实结论 §V5；与 SDK 单体调用的对拍验证仍属 M0-2 硬件环节。
...
**N2（安全增益，建议采纳）**：电机固件自带硬件看门狗 `motor_timeout_ms`，超时未收到新指令则**进入阻尼模式**；取值 `[0, 32760]` ms，**当前配置为 `0`＝禁用**（`6dof_Panthera_params_follower.yaml:5`）。
→ 这是一层**位于 armd 之下、armd 进程整体死亡也仍然生效**的硬件兜底，严格强于 §2 Mode C 的纯软件新鲜度窗口。建议设为略大于控制周期、略小于 jog 新鲜度窗口的值，并写入 §4 停止策略表作为最后一道防线。注意 `set_timeout()` 有 ~50ms 阻塞，只能 init 期调用。
...
→ **处置**：armd 必须检测重连（如监控 `get_motors()` 身份变化 / 状态 `time` 长期不更新）并重新 `get_motors()` 刷新缓存；此项应加入 M1 验收。

**N5**：`send_get_motor_state_cmd()` 在旧固件（`fun_v < fun_v2`）分支下会 **给所有电机下发 `velocity(0.0)` 再 flush**（`robot.cpp:599-606`）——「读状态」在旧固件上是**带副作用的写**。armd 须在启动时确认固件版本，否则周期性状态刷新会持续打断控制。
