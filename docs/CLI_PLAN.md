...
> **(c) 显式标注"v2 扩展"**（非机械臂 SDK 清单内、但架构文档要求的能力；D405 已由独立 `camera.proto` 落地，LeRobot 仍待 `dataset.proto`）。
>
...
| # | 方法（继承层，签名已核实） | 阶段 | CLI | rpc | 模式 | 备注 |
|---|---|---|---|---|---|---|
| S1 | `set_stop()` | v1 | `estop trigger` | `EStop` | 立即返回，最高优先级直通（绕过排队） | EStop 必须在 gRPC asyncio 层之前/旁路特殊处理，不与其它指令排队 |
| S2 | `set_reset_zero()` / `set_reset_zero_motors(ids)` | v1 | `calibrate zero` | `SetZero` | 立即返回 | 返回 void；重定义当前位置为零点，不产生目标运动。指定电机 binding 接收零基索引，RPC 1..7 在后端减一；仍要求 `--confirm` |
...
panthera control heartbeat [--interval FLOAT=0.5]
...
panthera cartesian plan-preview --waypoints TEXT
    示例: panthera cartesian plan-preview --waypoints "0.3,0,0.3,0,0,0;0.35,0.1,0.3,0,0,0"

panthera execution watch EXECUTION_ID
panthera execution cancel EXECUTION_ID
...
// 相机与数据能力已分别迁移到独立 camera.proto / dataset.proto，
// ArmService 不再保留占位 message 或 rpc。
...
|---|---|---|---|
| **v1** | 监控 + jog + moveJ/moveL + 夹爪 + 安全（含 EStop/回零） | **27** | #1-11,13,15-21,23,25,28 + S1,S2 |
| **v2** | 阻抗/MIT + 动力学诊断 + 多点轨迹 + 拖动示教录制回放 + 相机/LeRobot占位 | **18** | #12,14,29-41 + 占位2项 |
| 不暴露（有理由） | 内部实现细节 / 纯工具函数 / 已被封装 | 8 | #18,22,24,26,27,+电机层(说明性,非计数) |
| 生命周期 | 构造函数 | 1 | #42 |

**合计 42 项 SDK 能力均有归宿。27+18=45 条 CLI 操作中还包含 lease、EStop、execution 等架构能力；命令数量不再与 SDK 方法数量绑定。33 项直接能力、8 项不暴露与 1 项生命周期均有明确理由。**
...
- **M4 笛卡尔与运动学**：`kinematics fk/ik/jacobian/manipulability`、`cartesian movel/plan-preview`、`safety check-reached`。
  ...
  **v1 验收总口径**：27 条命令全部可用，且上述 4 组场景全部通过后，v1 视为完成。
...
1. **真机 v2 运动验收**：MIT、笛卡尔 jog、徒手示教阻力与真实轨迹回放误差仍须用户在场逐次确认；实现与仿真门禁已完成。
2. **LeRobot 首次隔离环境准备**：常驻 armd 不依赖 Torch/LeRobot；第一次导出会由 `uv --with lerobot` 准备隔离环境，因此需要网络或预配置 `PANTHERA_LEROBOT_RUNNER`。
3. **D405 视频流 / LeRobot 导出不计入 42 项机械臂 SDK 清单**：二者已由独立 `camera.proto` / `dataset.proto` 实现，审计时单列，不与机械臂方法数混计。
4. **安全收尾已定案**：`moveL`、多点轨迹和 `teach play` 统一使用 12 个控制周期减速，再进入安全保位/阻尼状态。
