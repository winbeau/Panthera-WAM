...
if ! command -v cmake >/dev/null 2>&1 || ! command -v pkg-config >/dev/null 2>&1; then
    echo "cmake/pkg-config is missing; install the WSL build dependencies first" >&2
    exit 1
fi
if ! pkg-config --exists libusb-1.0; then
    echo "libusb development files are missing; run:" >&2
    echo "  sudo apt-get install -y build-essential libssl-dev libusb-1.0-0-dev pkg-config" >&2
    exit 1
fi
