...
| 参数 | 值 | 说明 |
|---|---|---|
| `MANUAL_CLUTCH_KP_HOLD` | `[4, 60, 20, 2, 2, 1]` | 锁位刚度（J2 承重轴 60：质心远离时防下坠，kd 自动 ≥4.8；J3 20；偏移≈残差/kp） |
...
`lock` 现场验证：J2 kp=20 时高位约 6 秒零速度保持；2026-08-16 质心远离 J2 时
观察到下坠，J2 已提升到 kp=60（阻尼自动升到 4.8）；若仍缓慢漂移，属重力前馈残差，
